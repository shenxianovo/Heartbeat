...
| 窗口与标题 | Accessibility 焦点窗口/标题通知 | 前台、最小化及窗口名称 WinEvent | 恢复焦点窗口/标题通知；取消近期点击门控，实际通知直接建立新区间 |
| 标题转场规则 | 共用活动模型：同应用标题变化要求最近 1 秒内点击；窗口变化是否无条件切段受配置影响 | 同左 | 标题属于应用区间明细；不采集页面 URL；不保存持久窗口身份 |
...
| 区间与交接 | 共用 System 模型与 AppMonitor，30 秒快照；ingress、SDK outbox、Runtime 及旧格式兼容 | 同一共用实现 | 四 Track 共用内存缓冲与 Hub 批量交接；临时断采阈值默认确认间隔两倍；恢复不跨未知空白续接；Hub 前持久化及完整保留保证不在本轮范围 |
