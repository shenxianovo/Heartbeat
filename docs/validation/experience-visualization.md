...
`npm run verify` 通过：类型检查、ESLint、Prettier、22 项 Vitest 和生产构建。`npm run test:e2e` 验证 14 项 Chromium 用例，覆盖：
...
浏览器测试模拟认证和记录 API，验证读取契约及页面交互，不等于真实采集到 Web 的联调。此次仅修改前端及文档，后端和 Collector 沿用上一轮通过的验证结果。截图可由 `tests/e2e/replay.spec.ts` 重现到 `test-results/experience-expanded.png`、`replay-mobile.png` 等文件。
