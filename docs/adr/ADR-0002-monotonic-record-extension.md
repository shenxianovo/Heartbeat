...
适用范围更新（2026-09-14）：[ADR-0006](ADR-0006-result-correction-semantics.md) 已确认底层模型需要表达同一份结果的更正，包括内容和时间的更正。下面的单调延长规则保留用于持续状态续期，不再作为所有 Record 的通用更新边界；当前代码仍执行下面的有限更新规则，通用更正尚未实现。
