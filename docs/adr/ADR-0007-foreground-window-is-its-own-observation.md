...
> 注记（2026-09-17）：上面这条两倍说的是本决策之前的状态，不是当前实现。实现取值是采样间隔的三倍，见 [`CollectorOptions.cs`](../../src/Collectors/Heartbeat.Collector.Desktop.Mac/CollectorOptions.cs) 的 `maximum-gap-seconds` 默认值，理由就写在下面决策第 4 条。查倍数请以那一条和代码为准。
