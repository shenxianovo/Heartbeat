...
提问器输入 = 当日投影（把手、时长、共现簇）+ Strand 库，本质是一次 **diff**：今天的簇减去已被 Strand 或 Mute 裁决的部分 = 疑惑。**不根据 recap 叙事发问**——散文是有损派生物、随时可弃，在其上建知识 = 在缓存上盖楼。

- **闸**：复现 + 有意义时长（≥ recap 噪声地板，复用 ADR-023 <60s 丢弃纪律），不发明第二把尺。
- **封顶**：每天最多端 1–3 个，按"未解释时长"排序取头部，永不刷屏。
- **问题引用具体把手簇**（"{hyperframes-workspace, localhost:5173, blender, AfterFX} 今天共现 4.2h，是一件事吗？"），所以回答落库时挂哪些把手是确定的。
- **聚簇看时间贴邻，不看裸同日**：提案卡预勾的成员须与锚点段**分钟级交错**（同一注意力语境），仅"同一天出现过"不入簇——防止同日无关活动（如交错打的游戏）被吸进项目提案。启发式猜错由表单去勾兜底，纠正随该把手的自锚/Mute 一次性生效。

### 5. 确认走表单，确定性两端夹薄 LLM

裁决走**表单式**：AI 看着簇一次性给结构化提案卡（猜名字、猜释义、预勾成员 Handle），用户直接改字段、勾/去勾成员、打勾提交。"纠错"= 编辑字段，不是聊天。

保住 ADR-023 的对称：**确定性两端（提问 diff + 提交写库，可单测）夹一层薄 LLM（出提案，不可测但薄）**。多轮对话式纠错**留门不预建**——真碰到需来回澄清再长"展开讨论"入口。

**无会话态持久化**：因为提问器是无状态 diff，一场没答完的确认，那个簇还没被裁决，下次 diff 自然再端上来——diff 本身就是"续上"机制。

### 6. 反哺注入投影层；缓存读时判脏、不自动重生成

- **注入点 = 投影层**：投影搭骨架时把每个簇解析成 Strand（锚点主导，确定性），吐"这簇 = HyperFrames（释义…）"的带注结构，prompt 渲染名字。解析留在可测那一半。
