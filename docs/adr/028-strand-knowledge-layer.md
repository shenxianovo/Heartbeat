...
一个 **Strand** = 名字 + 自由释义 + 一组成员 **Handle**（它的可观测指纹）。单 Handle 是退化形态，等价于一条实体释义（"花生 = 敏毕设"）——实体释义与项目理解是**同一个类型**，不搞两套系统。
...
**提问单元 = 单个高特异性把手，不是共现簇（2026-07-19 真实数据修正）。** 初版设计成"把一天的共现把手聚成 constellation，问'这簇是一件事吗'"，被真实数据证伪：真实的一天没有空闲缝可断会话（explorer / 微信 / 浏览器每几分钟前台交错），聚簇把整天塌成一个 40 把手的巨簇，喂 LLM 得到"武汉大学学生的一天"这种零信息量提案，用户一看就走。改为**每个把手独立评分、一个把手一个问题**（"www.huasheng.cn 今天 2 小时，这是什么？"），回到愿景原话"问花生是什么"。为此 §3 的 Anchor/Satellite 强度**必须在 v1 就落地**（初版推迟是错误）：

- **哨兵剔除**：OS 外壳 / 系统 chrome（explorer、ShellExperienceHost、ApplicationFrameHost、__away__、newtab、localhost…）零身份信息，硬编码表剔除，绝不当锚点。
- **特异性先验（冷启动）**：browser 域名 / vscode 仓库根（3）≫ 裸系统 app 名（1）。
- **ubiquity 惩罚**：14 天回看里天天出现的把手（微信 / QQ / chrome）判为复现——特异性扣 2、时长 gate 从 30min 抬到 2h。无处不在的基础设施自然沉底。
- **gate**：非复现把手单日 ≥30min、复现把手 ≥2h（复用 ADR-023 <60s 噪声地板作下限）。
- **封顶**：每天最多 3 个，按 特异性 × 未解释时长 排序取头部。
- 与锚点时间贴邻的其它高特异性把手作**共现提示**（≤3，只喂 LLM 消歧，非待勾成员集）。
