...
Owner 从已验证令牌的 `sub` claim 取得，必须是 UUID。客户端不能提交 Owner、Timeline、Track 时间模式、服务端接收时间或其他服务端字段。
...
- 请求只包含 `type` 与 `version`；时间模式来自代码协议注册表。
- `(collector_id, type, version)` 是唯一地址。重复和并发获取复用已有 Track。
- 未知协议或版本返回 `400 unsupported_protocol`。
- 已有 Track 时间模式与当前协议定义不一致时返回 `409 track_protocol_conflict`。
...
- 当前上传只支持代码协议明确允许续期的 `range + explicit` 持续状态协议。
...
| `invalid_record` | ID、时间或载荷不符合协议；未写入。 |
| `conflict` | 同一个 ID 的固定字段与已存记录不同；未写入。 |
...
Collector 只能用 `stored` 回执确认对应项的上传进度。旧请求回执只确认发送时的快照，不能清除同一 Record 后来产生、尚未确认的新进度。
