...
| Hub 在线管理 | Web `/hubs`、页头“Hub”入口、最近 60 秒收发平滑曲线（接收/发送/确认） | `scenario hubs-fixture`；`verify changed` 的 Hub/API/Desktop 测试 | [Hub 管理](../../../../../docs/hub-management.md)、[服务器运行](../../../../../src/Server/README.md) | 浏览器为 fixture，检查 uPlot 曲线、回执延迟、断连、重启和减少动态；Hub/API 集成检查活动来源与 Owner/会话隔离；VRChat 使用模拟外部 API，真实登录和原生远程启停需人工验收 |
