...
- `Heartbeat.Collection.Headless` 是 Generic Host 控制台 composition：从本地 Package/Instance 配置托管 Account Subject 的 ManagedProcess Collector，先 drain 子进程再执行 Hub 终态上传；其服务器 Machine 身份只用于运维与认证，不成为 Fact Subject
