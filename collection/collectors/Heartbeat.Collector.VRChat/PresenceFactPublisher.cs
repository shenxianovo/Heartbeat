...
internal sealed class PresenceFactPublisher(Func<Guid>? idGenerator, Guid collectorId)
{
    ...
    public VRChatPresenceFact? Current => _current;

    public IReadOnlyList<VRChatPresenceFact> Publish(
        ...
        DateTimeOffset observedAt)
    {
        ...
        if (_current is not null && continues)
        {
    ...
    private static VRChatPresence ToPresence(VRChatPresenceFact fact) =>
        new(fact.WorldId, fact.WorldName, fact.InstanceId, fact.ObservedAccountId);
