...
public sealed class CollectorPageLayoutTests
{
    ...
    [Fact]
    public Task BrowserCollector_UsesBrowserActionsAndKeepsDiagnosticsCollapsed() =>
        Session.Dispatch(() =>
        {
            var state = new ViewModels.FakeDesktopState
            {
                Current = DesktopStateSnapshot.Empty with
                {
                    BrowserCollector = new BrowserCollectorState(
                        ...
                        true,
                        ExternalHostRuntimeStatus.Waiting,
                        "等待浏览器建立连接。",
                        false,
                        null)
                }
            };
            using var viewModel = ViewModels.TestViewModel.Create(state);
            viewModel.NavigateCommand.Execute("Collectors");
            var window = new MainWindow(viewModel)
            {
                Width = 900,
            ...
            var diagnostics = Assert.Single(
                browserCard.GetVisualDescendants().OfType<Border>(),
                border => border.Classes.Contains("browser-diagnostics"));
