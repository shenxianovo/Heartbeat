...
**采集边界随需求生长，不预建。** 愿景（"x年前的今天我在做什么"）的产品承诺是如实回放**数字活动**；非 PC 数据源（手机、日记、照片）在 Subject/Fact/Source 模型下可接入，但只在某类空白真实造成困扰时才建对应采集器。当前阶段聚焦把 PC 采集做深。

## Contexts

| Context | Directory | Responsibility |
|---------|-----------|----------------|
| Collection | `collection/` | 监听前台窗口切换与各应用内活动，生成使用记录，上传至服务端。`hub/` 是可复用的 Collector Runtime，**多实例星形直连 Analytics、不嵌套**（ADR-032）；`desktop/` 是桌面实例（含 system Collector 与通用 ExternalHost loopback 监听，不认识任何具名可选 Collector，ADR-049）；`collectors/` 存放 Browser、VRChat 等独立 Collector。server 旁的无头 host 以一个 Runtime 托管多个账号级 Instance，不携带桌面概念。 |
| Analytics | `server/` | 接收带 Revision 的事实快照，持久化活动数据，生成报表与叙事知识投影 |
