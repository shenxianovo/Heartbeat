...
后续信息对同一份观测结果的修正，而非新增一份独立结果；修正可以涉及历史结果的内容或时间范围，包括数值变小、区间缩短或时间位置改变。（语义已定，尚未实现，见 [未决设计·历史纠错与删除](docs/recording-open-questions.md#历史纠错与删除)）
_避免使用_: 新增独立观测、数值累加
...
**Device Identity**:
Heartbeat 中用于跨 Collector 关联同一台设备的稳定身份，不随系统重装改变。（语义已定，尚未实现，见 [未决设计·设备关联](docs/recording-open-questions.md#设备关联)）
