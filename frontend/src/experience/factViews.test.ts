...
export function fact(overrides: Partial<ExperienceSegment> = {}): ExperienceSegment {
  return { id: 'one', streamId: 'stream', factId: 'fact', revision: 1, targetKind: 'device', targetId: 1, deviceId: 1, targetName: 'Mac', source: 'system', appId: 1, appIdentityId: 1, appName: 'Browser', appKey: 'browser',
...
describe('Fact Views', () => {
  ...
  it('groups System by direct device Target and correlates Browser on that device', () => {
    ...
    const browser = fact({ id: 'browser', source: 'browser', deviceId: 42, targetKind: 'device', targetId: 42 })
    const wrongDevice = fact({ id: 'wrong-device', source: 'browser', deviceId: 43, targetId: 43 })
  ...
  it('keeps all related browser candidates, matching AppIdentity rather than product name', () => {
    ...
    const wrongDevice = fact({ source: 'browser', targetId: 2, deviceId: 2 })
    const wrongIdentity = fact({ source: 'browser', appIdentityId: 2, appId: 2 })
...
it('keeps unknown historical observations readable without treating their stream as an object', () => {
  ...
  const browser = { ...unknown, id: 'browser', source: 'browser' }
