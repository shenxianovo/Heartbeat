...
.target { padding: 10px 0; border-top: 1px solid var(--glass-border); }
.lane-row { display: grid; grid-template-columns: var(--label-width) minmax(0, 1fr); gap: var(--lane-gap); align-items: start; }
.target-summary { position: sticky; top: 0; z-index: 1; background: var(--background); border-radius: 6px; }
.target-label { min-width: 0; padding-top: 11px; }.target-name { display: flex; align-items: center; gap: 7px; width: 100%; text-align: left; }
.target-name svg { color: var(--muted-foreground); flex-shrink: 0; }.target-name strong { font-weight: 550; font-size: .85rem; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.device-toggle { cursor: pointer; }.chevron { transition: transform .15s; }.chevron.open { transform: rotate(90deg); }
.target-label > span { display: block; font-size: .65rem; color: var(--muted-foreground); margin: 4px 0 0 26px; }
