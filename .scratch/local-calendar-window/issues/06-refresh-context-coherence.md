...
- 自动验证：`npm test`（35 files / 246 tests）、`npm run build`、`dotnet test Heartbeat.slnx --no-restore`（884 tests）、`dotnet build Heartbeat.slnx --no-restore`（0 warnings / 0 errors）、`dotnet format style Heartbeat.slnx --diagnostics IDE1006 --verify-no-changes --no-restore`、聚焦 `vue-tsc -b` 与 `git diff --check` 全部通过。
