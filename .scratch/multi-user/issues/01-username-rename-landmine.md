...
多用户设计（2026-07-15 grilling session）决定：URL 用 username（GitHub 式 `base/:username`），
...
1. **旧链接腐烂**：`base/alice` 改名后本地缓存 stale，本人不登录就一直指向旧名。
2. **串号**：若 AuthService 允许旧名被他人重新注册，`base/alice` 本地查到老用户的行，
