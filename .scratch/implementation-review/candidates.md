...
> **2026-09-17 更新**：X 分区（横切）已经做完一轮，已完成的工作单已清理；剩余延期项在 `.scratch/x-series-hardening/issues/06-deferred.md`。逐条状态见下面「横切」小节前的说明。C / S / W 三个分区按作者判断顺延，本文其余内容仍是 `27cdf6f` 时的原始体检结论。
...
**本轮处理状态（2026-09-17，已完成的工作单已清理）**：下面的条目描述保持体检当时的原文不改，处理结果只在这里汇总。
