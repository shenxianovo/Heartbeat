...
后续线上证据：run 34325214843 使用 c2fb3a2，测试/镜像构建成功，部署健康等待结束时数据库仍在
迁移；08:02:24 UTC 记录完成，耗时 767.3 秒。重启确认没有待执行迁移。App Catalog 启动计数
超时导致重启，之后恢复；08:15:10 UTC 的公开周报请求在 ReportService.AggregateAsync 超时。
Owner 现报告服务与查询恢复，但尚无证据确认统计信息、缓存或资源压力中的具体根因。
线上迁移成功，10 分钟预算未满足，启动/查询性能与部署等待修复仍未完成；不标记整体 done。
