...
模型一致性检查通过。替换已按 Owner 授权提交为 `ae30045`。
随后仅从本地原快照做只读 pg_dump，在独立副本完成迁移与逐行核对；未迁移原库或部署。
迁移按 Owner 要求拆成六个 SQL 阶段，仍由同一事务保护；晚期失败整体回滚测试通过。
PostgreSQL 512 MiB / 0.75 CPU、Analytics 256 MiB / 0.25 CPU 下，迁移 37.4 秒，
备份到健康 48.337 秒，重启到健康 5.534 秒。220,146 / 1,891,698 行、聚合及重启核对均零差异。
详细证据见 issue 01；这轮演练修复随本次提交收敛。下一步是实际整机/磁盘余量、部署整体停服预算、
备份保留策略与实际客户端升级验收，整体不是 done。
