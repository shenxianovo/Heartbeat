...
- [x] 在独立完整备份副本逐条核对，并测受限资源下的迁移、重启和空间采样；实际整机/磁盘预算仍待部署验收。
...
2026-09-09 完整副本演练通过：220,146 条 Segment、1,891,698 条 Event 及重启后逐行零差异；
聚合也一致。数据库/Analytics 分别限制 512 / 256 MiB、0.75 / 0.25 CPU，迁移 37.4 秒，
备份至健康 48.337 秒，重启 5.534 秒。证据见 issue 01；实际整机、恢复策略与现场升级仍待完成，
不将副本演练等同于生产验收。
