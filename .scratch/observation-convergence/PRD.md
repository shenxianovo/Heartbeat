...
2026-09-11：根据本次模型讨论重新打开本 PRD，以承接同一功能的剩余架构收敛。
上一轮完成的是对象字段、五表与查询贯通；尚未完成新事实对旧交付模型的独立。
本轮规格已形成，实现与运行验证尚未开始；历史验证记录保留在 Comments，不作为本轮通过证据。
...
- [ ] 无旧 Subject/Stream 的新 Segment/Event 通过真实入口完成保存、重传、修订和读取。
- [ ] 原生必填、同 Fact 固定字段、Revision 与关系原子性在 Runtime 和存储一致执行。
- [ ] DTO、实体、数据库及所有活跃读写消费者解除旧交付键的强制依赖。
- [ ] System、Browser、VRChat、SDK、协议、Runtime 和缓存实际切换，含 Browser 单调 Revision。
- [ ] 离线、重启、进行中事实、迟到 ACK、Gap、旧缓存及包回退保管通过。
- [ ] 旧身份接管、历史未知、完整内容、追加迁移与失败重试保全通过。
- [ ] 查询、分析、Dashboard、本人关联、产品维护及准确设备归属通过。
- [ ] 相关回归、仓库规定检查、规范与规格审查完成；全库残余依赖盘点无未承接主路径。
- [ ] 所有保留兼容有消费者与退出条件，文档/任务状态同步；人工验证缺口有步骤、负责人和状态。
